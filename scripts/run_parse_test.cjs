...
try {
  ...
  // If dependencies are not installed, provide a helpful error
  console.error('Missing dev dependency: please run `npm install chardet iconv-lite --save-dev`');
...
try {
  ...
  if (encLower === 'gb2312') encoding = 'gbk';
  if (encLower.includes('gb18030')) encoding = 'gb18030';
  if (encLower.includes('utf-8') || encLower === 'utf8') encoding = 'utf8';

  try {
    text = iconv.decode(raw, encoding);
  } catch (e) {
    console.warn('iconv failed to decode with', encoding, '- falling back to utf8.');
