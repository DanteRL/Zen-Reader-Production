...
export const parseChapters = (text: string): Chapter[] => {
  ...
  // If too few detected headers, try paragraph-based detection before paging
  if (headerLines.length < 2) {
    const RAW_CHARS_PER_PAGE = 5000;

    // Split by 2+ newlines to get logical paragraphs/blocks
    const paragraphs = normalized.split(/\n{2,}/).map(p => p.trim()).filter(Boolean);

    // Build paragraph char indices in the normalized string
    ...
    // Heuristic: treat short paragraph followed by a long paragraph as a heading
    ...
    for (let i = 0; i < paragraphs.length - 1; i++) {
      ...
      if (cur.length > 0 && cur.length <= 120 && next.length >= 200) {
        // Likely a title/section header
        paraHeaders.push({ paraIndex: i, title: cur.split('\n')[0].trim(), charIndex: paraCharIndex[i] });
      }
    }

    // Also scan paragraphs for explicit heading patterns (Markdown / 第X章 / Chapter)
    ...
    // Prefer explicit headers, else paragraph-detected headers
    ...
    if (detectedParaHeaders.length >= 2) {
      // Build chapters from paragraph headers
    ...
    // Title generation helper (tries headings inside page, then first short line)
